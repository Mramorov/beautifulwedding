...
/**
 * Meta field: Wedding Gallery for svadba CPT
 */
if (!defined('ABSPATH')) {
    exit;
}

function svadba_add_gallery_metabox()
{
    add_meta_box(
        'svadba_gallery',
        'Wedding Gallery',
...
add_action('add_meta_boxes', 'svadba_add_gallery_metabox');
                ...
                frame.on('select', function() {
                    ...
                    attachments.forEach(a => {
                        if (!ids.includes(a.id.toString())) {
                            ids.push(a.id);
                            $('#svadba_gallery_preview').append(`<div class="gallery-image"><img data-id="${a.id}" src="${a.sizes.thumbnail.url}" /><button type="button" class="remove-image">×</button></div>`);
                        }
                    });
...
function svadba_save_gallery_meta($post_id)
{
    ...
    if ($post_type !== 'svadba' && $post_type !== 'service') return;
...
add_action('save_post', 'svadba_save_gallery_meta');
