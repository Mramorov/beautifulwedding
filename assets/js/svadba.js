(function ($) {
  ...
  function calculateSum() {
    ...
    // Стоимость всех дополнительных услуг (БЕЗ вычета стандартного авто) - будет выведена в "Сумма дополнительных услуг"
    var autoCostFull = 0;
    var autoDeductionToApply = 0;
    
    // Вычет из общей суммы делаем только когда изменены авто или время относительно базового значения
    if (!(isBaseAuto && selectedHours === distance)) {
      autoCostFull = selectedAutoPrice * selectedHours;
      autoDeductionToApply = baseAutoDeduction;
    }
    servicesSum += autoCostFull;
    ...
    // Вычет применяется только при расчете общей суммы
    var totalSum = originalBase + placeAddition + servicesSum - autoDeductionToApply;
  ...
  function handleMapModal() {
    var $mapOverlay = $('#mapModalOverlay');
    var $mapModal = $('#mapModal');

    if (!$mapOverlay.length || !$mapModal.length) {
      return;
    }

    function openMapModal() {
      $mapOverlay.addClass('active');
      $mapModal.addClass('active').attr('aria-hidden', 'false');
      $('body').addClass('no-scroll');
    }

    function closeMapModal() {
      $mapOverlay.removeClass('active');
      $mapModal.removeClass('active').attr('aria-hidden', 'true');
    ...
    $(document).on('click', '#closeMapModal, #mapModalOverlay', function () {
      closeMapModal();
    });
